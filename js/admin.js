...
function triggerManualDateRollover() {
    if (!confirm('Shift all "Today" results into "Yesterday" and reset "Today" for a new day?')) return;
    ...
    if (typeof checkDateRollover === 'function') {
        checkDateRollover();
    } else if (typeof autoSyncTodayResults === 'function') {
    ...
    showToast('New day started! Today results shifted to Yesterday.', 'success');
}
...
function getChartKeys(chartIndex) {
    var maps = [
        ...
        // Yearly record charts (indices 5–8)
        { header: 'year_headers', data: 'year_2026_data' },
        { header: 'year_headers', data: 'year_2025_data' },
        { header: 'year_headers', data: 'year_2024_data' },
        { header: 'year_headers', data: 'year_2023_data' }
    ];
...
function getChartContainerId(chartIndex) {
    var ids = [
        'admin-chart1', 'admin-chart2', 'admin-chart3',
        'admin-fullchart', 'admin-prev-fullchart',
        // Yearly record charts
        'admin-year-2026', 'admin-year-2025', 'admin-year-2024', 'admin-year-2023'
    ];
...
// --- Record Games List Editor ---

function renderAdminRecordGamesTable() {
    var container = document.getElementById('admin-record-games-table');
    if (!container) return;
    ...
    games.forEach(function (game, i) {
        ...
        html += '<td class="editable-cell" onclick="editRecordGameCell(' + i + ',\'slug\',this)" title="Click to edit slug">' + (game.slug || '') + '</td>';
        html += '<td class="row-actions"><button class="btn-admin btn-danger btn-xs" onclick="deleteRecordGameRow(' + i + ')">✕</button></td>';
        html += '</tr>';
    });

    html += '</tbody></table>';
    html += '<div class="mt-10"><button class="btn-admin btn-success" onclick="addRecordGameRow()">+ Add Record Game</button></div>';
    container.innerHTML = html;
}
...
function editRecordGameCell(index, field, cell) {
    makeEditable(cell, function (newValue) {
        ...
        // Re-render yearly charts in admin
        renderAdminChart('admin-year-2026', 'year_headers', 'year_2026_data', 5);
        renderAdminChart('admin-year-2025', 'year_headers', 'year_2025_data', 6);
        renderAdminChart('admin-year-2024', 'year_headers', 'year_2024_data', 7);
        renderAdminChart('admin-year-2023', 'year_headers', 'year_2023_data', 8);
        showToast('Record game updated!');
    });
}

function addRecordGameRow() {
    var name = prompt('Enter new record game name:');
    if (!name || !name.trim()) return;
    var games = getData('record_games') || [];
    var newSlug = name.trim().toLowerCase().replace(/\s+/g, '-');
    ...
    renderAdminChart('admin-year-2024', 'year_headers', 'year_2024_data', 7);
    renderAdminChart('admin-year-2023', 'year_headers', 'year_2023_data', 8);
    showToast('Record game added!');
}

function deleteRecordGameRow(index) {
    var games = getData('record_games') || [];
    var gName = games[index] ? (games[index].name || games[index].display) : ('Game ' + (index + 1));
    if (!confirm('Delete record game "' + gName + '"?')) return;
    games.splice(index, 1);
    setData('record_games', games);
